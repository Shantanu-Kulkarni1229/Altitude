module.exports = (customerMessage, { hasActiveTrek = false } = {}) => `
...
7. isInfoRequest: boolean (true/false). Set to true if the customer is asking a follow-up question about a trek already being discussed, rather than describing new preferences or starting a new search — this includes explicit questions ("tell me more", "what's the itinerary", "is it safe", "what altitude does it reach", "what's included") AND short reactions/prompts that only make sense as a follow-up ("why this one?", "why?", "how come?", "really?", "how long is it", "how much"). ${hasActiveTrek
    ? `A trek IS currently being discussed with this customer, so treat any short or ambiguous message that doesn't clearly introduce a new preference as an info request by default — err toward true, not false.`
    : `No trek is currently being discussed, so a bare question like "why?" has nothing to refer to — set this false unless the message is self-evidently about trekking.`
  }
