    ...
    return (
      <Link to={`/trek/${trek.trekId}`} className="flex gap-4 p-3 rounded-lg border border-stone-200 bg-white hover:border-stone-300 hover:shadow-sm transition-all group">
        ...
        <div className="flex-1 flex flex-col justify-center">
          ...
          <p className="text-sm font-medium text-stone-900">₹{trek.basePrice.toLocaleString()}</p>
        </div>
      </Link>
    );
  }
    ...
    <Link to={`/trek/${trek.trekId}`} className="group flex flex-col bg-white rounded-xl overflow-hidden border border-stone-200 hover:shadow-md transition-all duration-300">
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-xl font-semibold text-stone-900 line-clamp-1">{trek.name}</h3>
        </div>
        ...
        <div className="flex items-center text-sm text-stone-500 gap-4 mb-4">
          ...
          <span className="flex items-center gap-1.5"><Clock className="w-4 h-4" /> {trek.duration}</span>
        </div>
